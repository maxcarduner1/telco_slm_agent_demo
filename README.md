...
The app uses a Databricks control plane for supervisor behavior: the supervisor
prompt is loaded from MLflow Prompt Registry and the supervisor model is reached
through an AI Gateway endpoint. The app still has a local prompt fallback so it
can start if prompt loading fails, but healthy deployments should use the
registered prompt.
...
Databricks App
  ...
  SUPERVISOR_PROMPT_URI=prompts:/<catalog>.<schema>.telcogpt_supervisor@production

Control plane setup
  notebooks/09_setup_prompt_registry.py
    -> registers telcogpt_supervisor prompt
  notebooks/10_setup_gateway_endpoint.py
    -> creates telcogpt-supervisor AI Gateway endpoint
```

Current supervisor resources:

| Resource | Name |
|----------|------|
| App | `otel-telco-agent` |
| AI Gateway endpoint | `telcogpt-supervisor` |
| Prompt | `<catalog>.<schema>.telcogpt_supervisor` |
| App traces | `/Shared/otel-telco-agent-traces` |

### Future Releases

The Smart Model Upgrades optimizer is not implemented in this repo. Future work
can add a separate manual or scheduled optimizer workflow that evaluates prompt
and model candidates, writes results to MLflow, and only promotes after explicit
guardrails and rollback checks are in place.
...
│   ├── 08_grant_app_uc_permissions.py # App SP grants for UC/functions/indexes/prompts/endpoints
│   ├── 09_setup_prompt_registry.py   # Register supervisor prompt in MLflow Prompt Registry
│   └── 10_setup_gateway_endpoint.py  # Create supervisor AI Gateway endpoint
├── prompts/
│   └── supervisor.yaml               # Seed prompt registered to Prompt Registry
...
- MLflow Prompt Registry enabled for full Prompt Registry behavior. If it is
  disabled, the setup job skips prompt registration and the app falls back to
  the in-repo supervisor prompt.
...
| `telcogpt-supervisor` | Auto/manual setup via `notebooks/10_setup_gateway_endpoint.py`; routes to `databricks-claude-sonnet-4` through AI Gateway |
| `databricks-claude-sonnet-4` | **Manual dependency** — built-in PAYG model destination used behind the supervisor gateway |
...
| `create_vs_indexes` | `provision_endpoints` + `parse_documents` | Embeds chunks and creates 3 Delta Sync VS indexes |
...
| `setup_prompt_registry` | `setup_catalog_schema` | Registers/aliases the supervisor prompt as `<catalog>.<schema>.telcogpt_supervisor@production` |
| `setup_gateway` | — | Creates the AI Gateway endpoint `telcogpt-supervisor` pointing at `databricks-claude-sonnet-4` |
| `grant_app_uc_permissions` | UC functions, VS indexes, Lakebase app, prompt registry, gateway | Grants the app SP UC, prompt, gateway, serving endpoint, and Vector Search access |
